...
#ifndef WIN32
	// rss-port: use the client-side vertex array path under gl4es. The VBO
	// path (buffer-offset glDrawElements) is unreliable through the GL->GLES2
	// translation, and the surface is re-tessellated on the CPU every frame,
	// so VBOs offer no performance benefit here anyway.
	mUseVBOs = false;
#endif
...
impSurface::draw()
{
	...
	if (mUseVBOs)
	{
		...
		// Unbind the VBOs. Leaving them bound corrupts client-side vertex
		// array rendering done elsewhere (gl4es implements immediate mode
		// with internal vertex arrays, so a stale GL_ARRAY_BUFFER binding
		// blacks out all subsequent glBegin/glEnd drawing).
