...
#ifdef RS_XSCREENSAVER
void reshapeRS(int width, int height){
	xsize = width;
	ysize = height;
	centerx = width / 2;
	centery = height / 2;
...
void initSaver(){
	...
	reshapeRS(xsize > 0 ? xsize : 1024, ysize > 0 ? ysize : 768);
#endif
#ifdef WIN32
...
#ifdef RS_XSCREENSAVER
	if(dSound)
		soundengine = new SoundEngine(NULL, float(dSound) * 0.01f);
#endif
...
void reshape(int width, int height){
	reshapeRS(width, height);
}

void handleCommandLine(int argc, char* argv[]){
	setDefaults();
	getArgumentsValue(argc, argv, std::string("-maxrockets"), dMaxrockets, 1, 100);
	getArgumentsValue(argc, argv, std::string("-smoke"), dSmoke, 0, 40);
	getArgumentsValue(argc, argv, std::string("-explosionsmoke"), dExplosionsmoke, 0, 5);
	getArgumentsValue(argc, argv, std::string("-wind"), dWind, 0, 100);
	getArgumentsValue(argc, argv, std::string("-ambient"), dAmbient, 0, 100);
	getArgumentsValue(argc, argv, std::string("-stardensity"), dStardensity, 0, 100);
	getArgumentsValue(argc, argv, std::string("-flare"), dFlare, 0, 100);
	getArgumentsValue(argc, argv, std::string("-moonglow"), dMoonglow, 0, 100);
	getArgumentsValue(argc, argv, std::string("-sound"), dSound, 0, 100);
	getArgumentsValue(argc, argv, std::string("-moon"), dMoon, 0, 1);
	getArgumentsValue(argc, argv, std::string("-clouds"), dClouds, 0, 1);
	getArgumentsValue(argc, argv, std::string("-earth"), dEarth, 0, 1);
	getArgumentsValue(argc, argv, std::string("-illumination"), dIllumination, 0, 1);
}
#endif
